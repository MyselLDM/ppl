...
parse_tokens() // ---> AST_NODE_PROGRAM
        |
parse_statement() // ---> AST_NODE_STATEMENT (var, print, if, for, while, assignment)
        | 
        ---> parse_expression() ---> AST_NODE_EXPRESSION / AST_NODE_LITERAL / AST_NODE_OPERATOR
        |
        ---> parse_block() ---> AST_NODE_BLOCK (nested statements)
        |
parse_error()  // Called on invalid token/structure
        |
ast_create_node() / ast_add_child() / ast_free() / ast_print()
